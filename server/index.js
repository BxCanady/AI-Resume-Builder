...
app.post("/api/upload", upload.single("headshotImage"), async (req, res) => {
    try {
      ...
      // Extract API key from request payload
  ...
  async function generateResume(formData, apiKey) {
    try {
      ...
      let prompt = `Generate a resume for ${formData.fullName}.\n\n`;
      // Add more fields as needed to the prompt
